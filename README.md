...
| **Domoblock** | Fintech/tokenized real-estate investment platform. Designed and developed the entire backend — NestJS microservices (auth, KYC, payments, investments, blockchain, notifications) in hexagonal/DDD architecture over RabbitMQ, in an Nx monorepo — and led a team building it. Automated the end-to-end purchase flow — MangoPay eWallet integration, automated withdrawals, token purchases, and top-ups. On-chain token minting on Polygon. Deployed on Google Cloud Run. | `NestJS` `Angular` `RabbitMQ` `PostgreSQL` `Firebase` `Google Cloud Run` `Docker` |
